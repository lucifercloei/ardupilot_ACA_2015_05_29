...
static void land_run()
{
	...
	int16_t control_4 = g.rc_4.control_in; //Yaw
	int16_t control_3 = g.rc_3.control_in; //Throttle

	if(control_4 < -4000 && control_3 < 5)
	{
		init_disarm_motors();
		land_override = 1;
	}
	...
	//gcs_send_text_P(SEVERITY_LOW, PSTR("In land mode"));
    if (land_with_gps) {
        land_gps_run();
    }else{
        land_nogps_run();
