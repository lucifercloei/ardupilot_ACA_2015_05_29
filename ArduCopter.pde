...
/*
static int test_sonar_flag = 0;
static int test_auto_mode_print = 0;
static int test_gpio_flag = 0;
static char test_buffer_print[48];
*/
//************************* * * * * $ * * * * *************************//
// main loop scheduler
...
/*
	FPGA_read_state = hal.gpio->read(52)*4 + hal.gpio->read(51)*2 + hal.gpio->read(50);


	if(ai.lidar_cm < FPGA_LIDAR_THR_1)
	{
		hal.gpio->write(53, LOW); //(Lower bit)
		hal.gpio->write(54, LOW); //(higher bit)
	}
	else if (ai.lidar_cm < FPGA_LIDAR_THR_2)
	{
		hal.gpio->write(53, HIGH); //(Lower bit)
		hal.gpio->write(54, LOW); //(higher bit)
	}
	else if (ai.lidar_cm < FPGA_LIDAR_THR_3)
	{
			hal.gpio->write(53, LOW); //(Lower bit)
			hal.gpio->write(54, HIGH); //(higher bit)
	}
	else
	{
		hal.gpio->write(53, HIGH); //(Lower bit)
...
*/
    ...
    //************************* * * * * * * * * * *************************//
    int16_t lidar_distance =0;
    lidar_distance = sonar.distance_cm();

    ai.update_lidar(lidar_distance);
    if(ai.system_start)
    {
    	ai.update_ten_hz(millis());
    }
    //************************* * * * * $ * * * * *************************//
}
...
//************************* * * * * * * * * * *************************//
// dealing with all printing in one_hz_loop
void one_hz_print()
{
	if (land_override == 1)
	{
		gcs_send_text_P(SEVERITY_HIGH, PSTR("FORCE LAND"));
	}

	if(FPGA_start == 1)
	{
		switch(FPGA_read_state)
		{
			case 0:
				gcs_send_text_P(SEVERITY_HIGH, PSTR("hover"));
				break;
			case 1:
				gcs_send_text_P(SEVERITY_HIGH, PSTR("forward"));
				break;
			case 2:
				gcs_send_text_P(SEVERITY_HIGH, PSTR("up"));
				break;
			case 3:
				gcs_send_text_P(SEVERITY_HIGH, PSTR("down"));
				break;
			case 4:
				gcs_send_text_P(SEVERITY_HIGH, PSTR("left"));
				break;
			case 5:
				gcs_send_text_P(SEVERITY_HIGH, PSTR("right"));
				break;
			case 6:
				gcs_send_text_P(SEVERITY_HIGH, PSTR("not in use"));
				break;
			case 7:
				gcs_send_text_P(SEVERITY_HIGH, PSTR("avoidance sequence"));
				break;
		};
	}

	if(ai.system_start == 1)
	{
		static char gcs_buffer[25];
		char aca_state[100];
		sprintf(gcs_buffer,"%d**%d",ai.ACA_time_elapse,ai.ACA_time_elapse2);
		//print ai_state
		switch (ai.mode_curr)
			 {
				case 0 :
					strcpy (aca_state,"Sleep");
					break;
				case 1 :
					strcpy (aca_state,"Slow");
					break;
				case 2 :
					strcpy (aca_state,"Halt");
					break;
				case 3 :
					switch(ai.dir_curr)
					{
						case 0 :
							strcpy (aca_state,"STAY");
							break;
						case 1 :
							strcpy (aca_state,"LEFT");
							break;
						case 2 :
							strcpy (aca_state,"RIGHT");
							break;
						case 3 :
							strcpy (aca_state,"UP");
							break;
						case 4 :
							strcpy (aca_state,"DOWN");
							break;
						case 5 :
							strcpy (aca_state,"FORWARD");
							break;
						case 6 :
							strcpy (aca_state,"BACKWARD");
							break;
					}
					break;
			 }

		strcat(aca_state,gcs_buffer);
		if(ai.avoid_emerg == 1)
		{
			strcat(aca_state,"E");
		}

		gcs_send_text_P(SEVERITY_HIGH, PSTR(aca_state));
	}
	else
	{
		//gcs_send_text_P(SEVERITY_HIGH, PSTR("system sleep"));
	}
...
static void one_hz_loop()
{
    ...
    if (pre_arm_display_counter >= 30) {
        ...
        //gcs_send_text_P(SEVERITY_HIGH, PSTR("TEST A"));
    }else{
		...
		/*hal.gpio->pinMode(55, HAL_GPIO_INPUT);
         if (hal.gpio->read(55))
        {gcs_send_text_P(SEVERITY_HIGH, PSTR("HIGH"));}
        	else
        {gcs_send_text_P(SEVERITY_HIGH, PSTR("LOW"));}
         */

         /*
         sprintf(test_buffer_print,"%d",g.rc_2.control_in);
         gcs_send_text_P(SEVERITY_HIGH, PSTR(test_buffer_print));
		*/

         //************************* * * * * $ * * * * *************************//
        //else
        //{gcs_send_text_P(SEVERITY_HIGH, PSTR("clear"));}
        //end
