...
static void auto_wp_run()
{
    ...
    switch (ai.mode_curr)
    {
       case 0 :
           ...
           {
        	   //auto_init(0);
        	   //wp_nav.set_speed_xy (ACA_WP_SPEED_SLEEP);
        	   ai.reset_speed = 0;
           }
           auto_wp_sleep_slow_run();
           break;
       ...
       case 1 :
		   ...
		   if(ai.reset_speed)
		   {
			   ...
			   auto_init(0);
			   //wp_nav.set_speed_xy (ACA_WP_SPEED_SLOW);
			   ai.reset_speed = 0;
		   }
		   auto_wp_sleep_slow_run();
           break;
...
static void auto_wp_sleep_slow_run()
{
	// process pilot's yaw input
	    float target_yaw_rate = 0;
	    if (!failsafe.radio) {
...
static void auto_wp_avoid_run()
{
		float target_yaw_rate = 0;
	    float target_climb_rate = 0;

		// process AI roll and pitch input
		wp_nav.set_pilot_desired_acceleration(ai.rc1_input, ai.rc2_input);

		// get AI desired yaw rate
		target_yaw_rate = get_pilot_desired_yaw_rate(ai.rc4_input);

		// get AI desired climb rate
		target_climb_rate = get_pilot_desired_climb_rate(ai.rc3_input);
